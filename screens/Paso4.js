...
const Paso4 = ({ navigation, route }) => {
  ...
  const subirMastarde = async () => {
    ...
    if (pasoDesc === '') {
      ...
      await AsyncStorage.setItem('recetaGuardada', JSON.stringify({ "usuarioId": usuario._id, "usuario": usuario.name, "nombreReceta": nombreReceta, "descripcion": descripcion, "imagen": imagen, "personas": personas, "minutos": minutos, "esfuerzo": esfuerzo, "tipo": tipo, "pasos": [paso1, paso2, paso3,{ "paso": 4, "descripcion": pasoDesc, "image": image, "videoImage": videoImage }], "ingredientes": ingredientes }));

      setRecetaGuardada({ "usuarioId": usuario._id, "usuario": usuario.name, "nombreReceta": nombreReceta, "descripcion": descripcion, "imagen": imagen, "personas": personas, "minutos": minutos, "esfuerzo": esfuerzo, "tipo": tipo, "pasos": [paso1, paso2, paso3,{ "paso": 4, "descripcion": pasoDesc, "image": image, "videoImage": videoImage }], "ingredientes": ingredientes })
  ...
  const paso5 = () => {
    ...
    navigation.navigate('Paso5', {
      ...
      paso2:paso2,
      paso3:paso3,
      paso4:{"paso":4, "descripcion":pasoDesc, "image":image, "videoImage":videoImage},
      idReceta:idReceta,
      editar:editar,
  ...
  const finalizarCarga = async (event) => {
    ...
    try {
      if (reemplazar) {
        let replaceReceta = await ReplaceRecetaAPI(idReceta, usuario._id, usuario.name, nombreReceta, descripcion, imagen, personas, minutos, esfuerzo, tipo, [paso1,paso2,paso3,{ "paso": 4, "descripcion": pasoDesc, "image": image, "videoImage": videoImage }], ingredientes);
      ...
      if (editar) {
        ...
        let updateReceta = await UpdateRecetaAPI(idReceta, usuario._id, usuario.name, nombreReceta, descripcion, imagen, personas, minutos, esfuerzo, tipo, [paso1,paso2,paso3,{ "paso": 4, "descripcion": pasoDesc, "image": image, "videoImage": videoImage }], ingredientes);
      ...
      if ((editar == false) && (editar == false)) {
        ...
        let createReceta = await CreateRecetaAPI(usuario._id, usuario.name, nombreReceta, descripcion, imagen, personas, minutos, esfuerzo, tipo, [paso1,paso2, paso3,{ "paso": 4, "descripcion": pasoDesc, "image": image, "videoImage": videoImage }], ingredientes);
  ...
  useEffect(() => {
    ...
    (async ()=>{
      ...
      if(pasos.length>3){
        setPasoDesc(pasos[3].descripcion);
      setImage(pasos[3].image);
      setVideoImage(pasos[3].videoImage);
      }
  ...
  return (
    <View style={Global.container2}>
      ...
      <View style={styles.ingreMedidas} >
        ...
        <TouchableOpacity style={[styles.btn, Global.shadows]} onPress={paso5}
